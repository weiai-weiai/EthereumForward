    ...
    internal class SocketServer
    {
        ...
        public void Init()
        {
            try
            {
                ...
                serviceSocketListener.Bind(new IPEndPoint(IPAddress.Parse("0.0.0.0"), 20000)); //IP和端口应该是可配置
                serviceSocketListener.Listen();
                Thread handleSocket = new Thread(() => HandleSocket(serviceSocketListener)); ;
                handleSocket.Start();
        ...
        private void HandleSocket(Socket socket)
        {
            while (true)
            {
                try
                {
                    ...
                    serviceMessage.ProcessSocket(currSocket);
